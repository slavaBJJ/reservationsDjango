...
Arrêter le serveur avec `Ctrl+C` dans PowerShell.
...
dépendances ne sont pas installées. Réexécuter :
...
Vérifier que la commande utilise bien `.venv\Scripts\python.exe`, puis
réinstaller les dépendances. `psycopg2-binary` est déjà déclaré dans
`requirements.txt`.
...
`DJANGO_DATABASE` ou `POSTGRES_*` n'est définie dans PowerShell.

Pour retirer temporairement ces variables de la session courante :
